...
def get_ship_count_in_fleets(fleets):
    count = 0
    for f in fleets:
        count += f.size
    return count
...
def most_spawning_shipyard(shipyards):
    best_so_far = 0
    target = None
    for es in shipyards:
        if es.max_spawn > best_so_far:
            target = es
            best_so_far = es.max_spawn
    return target
...
class ShipyardManager:
    ...
    def get_unassigned_shipyards(self):
        result = []
        for s in self.shipyards:
            if not s.assigned:
                result.append(s)
    ...
    def excess_vs_conditional(self, board, s):
        amount = s.ship_count
        ...
        ######################This is key.  The longest available path is capped by conditional time ##################

        if s.excess < s.ship_count and s.excess < 34:

            max_l = min(s.get_conditional() - board.step, max_length)  # need board.step -1 here?
            ...
            if s.excess >= 8 and max_l >= 2:
                ...
                fp2, best_size2, expected_kore2 = sutils.find_best_path(ranked_paths2, s.ship_count, ships_min, ships_max,
                                                                        self.board, my_fleets,
                                                                        enemy_fleets, my_shipyards, enemy_shipyards,
                                                                        True)
                ...
                if CONDITIONAL_LOGGING:
                    print(
                        f"{self.board.step} {s.position} Found a conditional path: best_size: {best_size}, {s.ship_count} {fp.get_compact_path()} {max_l}")
    ...
    def assign_return_route(self, board, s):

        assert s.excess <= s.ship_count

        round_trip_debug = []
        round_trip_debug.append(s.ship_count)
        round_trip_debug.append(s.excess)
    ...
    def find_expo_location(self, pos):
        start = pos
        radius = 6
    ...
    def find_best_position_for_shipyards(self):
        ...
        for p in self.board.cells:
            ...
            scores.append({"score": nearby_kore, "point": p})
    ...
    def assign_reinforce(self):
        if len(self.shipyards) < 5: # expriment with this number and make it into a parameter
            return

        if self.me.kore < self.get_enemy().kore:
            return
        for s in self.shipyards:
            if s.ship_count > 45: # make this number into a parameter
                target = self.find_weakest_shipyard()
                if target is None:
                    return False

                if target.max_spawn < s.max_spawn:
                    task = {"type": "defend", "request_time": self.board.step, "reserved": s.ship_count,
                            "location": target.position, "reserve_type": 1, "priority": parameters.REINFORCE_PRIORITY, "id": s.id}
        ...
        for s in self.shipyards:
            ...
            for e in enemy_shipyards:
                ...
                if timeit.default_timer() - starttime > .5:
                    print(f"{self.board.step} The time difference is :", timeit.default_timer() - starttime)
    ...
    def assign_expanders(self, km):
        ...
        if candidate is not None:
            ...
            for i in range(0, parameters.MAX_WAIT_TIME_TO_EXPO):
                num = sutils.number_of_returning_ships(self.board, fleets, candidate.position, i) + s.ship_count
                spawn = self.me.kore // 10
                spawn = min(s.max_spawn * 4, spawn)
                if num + spawn >= 50:
                    turns = i
                    break
            if turns > parameters.MAX_WAIT_TIME_TO_EXPO:
                if EXPO_LOGGING:
                    print(
                        f"{self.board.step} Could not expo because no one was ready withing {parameters.MAX_WAIT_TIME_TO_EXPO} turns, could have been in {turns}")
                return False
    ...
    def attack_shipyard(self, s, task):
        reserved = task["reserved"]
    ...
    def get_shipyard_by_id(self, sid):
        for s in self.shipyards:
            if s.id == sid:
                return s
    ...
    def cannot_defend_abandon_ship(self):
        return False
    ...
    def assign_defenders(self, fi, km, board):
        ...
        attacking_fleets = sutils.under_attack(self.board)
        ...
        for f in attacking_fleets:
            ...
            if not self.can_we_defend(target_s, fleet, km):
                ...
                self.cannot_defend_abandon_ship()
                continue
    ...
    # to estimate max spawn ability better we should take into account that during time t the shipyards can spawn more
    def max_spawn_ability(self):
        ms = 0
        for s in self.shipyards:
            ms += s.max_spawn
        return ms

    def max_can_spawn_in_n_turns(self, n):
        amount = 0
        for s in self.shipyards:
            amount += sutils.amount_ships_can_spawn_in_n_steps(self.board, n, s)

        return amount

    def num_fleets_expanding(self, fleets):
        count = 0
        for f in fleets:
    ...
    def num_ships_expanding(self, fleets):
        count = 0
        for f in fleets:
            if "C" in f.flight_plan:
                count += f.ship_count
    ...
    def num_expansions_in_tasks(self):
        count = 0
        for s in self.shipyards:
            for t in s.tasks:
                if t["type"] == "expand":
    ...
    def should_expand(self, km):
        ...
        enemy = self.get_enemy()
        enemy_sys = len(enemy.shipyards) + self.num_fleets_expanding(enemy.fleets)
